  ...
  return (
    <main className="admin-main">
      ...
      <section className="detail-grid">
        ...
        <article className="detail-card"><h2>State history</h2>{data.state_history.length ? <ul>{data.state_history.map((item) => <li key={item.id}><strong>{item.from_state ?? "—"} → {item.to_state}</strong><span>{item.reason_code ?? "state change"} · {item.triggering_event}</span><small>correlation {item.correlation_id}</small></li>)}</ul> : <p className="muted">No state history yet.</p>}</article>
        <article className="detail-card"><h2>Event history</h2>{data.events.length ? <ul>{data.events.map((item) => <li key={item.id}><strong>{item.event_type} · v{item.event_version}</strong><span>{item.authority.toLowerCase()} · {item.producer}</span><small>correlation {item.correlation_id} · cause {item.causation_id ?? "root"}</small></li>)}</ul> : <p className="muted">No events recorded.</p>}</article>
